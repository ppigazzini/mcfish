...
| `malformed` | drives seven crafted `.rtbw` headers past the sanitized engine and judges the four parts of a refusal -- exit 0, no sanitizer report, a diagnostic naming the file, and it still answers -- plus a clean-corpus control that must stay silent | that a file refused yesterday is refused today. `signature` is green with every parser defect it covers LIVE (the bench reads no file the engine did not ship with) and `fuzz-tb` is probabilistic and nightly, so nothing else is a regression test for the bounds in `decode.c` and `registry.c`. A fixture is a GENERATOR, not a blob: the interesting thing is which field is wrong. 2.4 s, in `parity` |
