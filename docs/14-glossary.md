...
| **sibling** | `../zfish`, `../rfish`, `../fcfish`, and `../Stockfish`'s `refish` branch: peer ports of the same golden. The last one is a branch INSIDE the golden's checkout, so `git log` there answers for it unless you say `upstream/master`. **None of them is a source and none is behind another**; a finding in one is a hypothesis about this tree, to be probed here before it is fixed here. [AGENTS.md](../AGENTS.md) owns the rules |
