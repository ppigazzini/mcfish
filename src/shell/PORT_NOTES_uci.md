...
- **`engine.h` — an FEN accessor.** `uci_bench.c` reads the live position's FEN
  ...
  `engine_fen()`. Upstream's bench calls `engine.fen()` (uci.cpp:266). An
  `void engine_fen(char *buf, int buf_len)`
...
- **No output mutex.** Upstream serialises output through `sync_cout` so a search
  thread and the listener cannot tear a line. `build.sh` links no pthread and the
  search is single-threaded, so `uci_output.c` has none. The funnel is preserved
  — every line goes through `uci_output_print_line` — so adding the lock at M4 is
  a one-function change.
- **Bounded input line.** Upstream's `std::getline` is unbounded; `UciInput`
  holds `UCI_LINE_MAX` (64 KiB) and sets `truncated` on overflow, consuming the
  tail so the next read starts on a line boundary. 64 KiB is far past any
  reachable `position ... moves` line.
- **Whole-token integer parse.** `is >> limits.depth` stops at the first
  ...
  `uci_parse` requires the whole token and reports `bad_token`, which is what
  makes upstream's own "Invalid argument for 'depth'" path reachable at all; it
  differs from the golden only on malformed input.
...
- **`benchmark` / speedtest** (upstream `uci.cpp:311`). Its report needs the NUMA
  config string, thread-binding information, and the large-pages flag, none of
  which exist yet; the whole command lands with M4. `bench_positions.c` therefore
  carries only `Defaults`, not the separate benchmark position set that only
  `benchmark` reads.
...
- **`dbg_print()`** before the bench summary (uci.cpp:300). Upstream's
  `dbg_print` and its counters are unported; `misc.h` has no equivalent.
- **The critical-error path.** The formatter is ported —
