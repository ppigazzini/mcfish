...
//
// OUT OF LINE, as upstream's `TranspositionTable::probe` is (tt.cpp:254) -- an
// ordinary method its link-time optimizer keeps as one 331-byte symbol rather
// than folding into the node bodies. Forcing it inline here put a copy of the
// cluster loop, and the register pressure that comes with it, inside each of the
// three search clones and both qsearch clones.
