...
|---|---|---|
| `test_main.c` | mcfish | **Yes** — this is the unit + property suite, run by `./build.sh test` |
...
and **do not run against mcfish today** — mcfish has no NNUE net, no Syzygy
support, and a different bench signature. Do not wire them into `build.sh`
expecting them to pass.

`scripts/` is the same: an upstream mirror, not mcfish-owned.
