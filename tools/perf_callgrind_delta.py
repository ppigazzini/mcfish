...
own header says "SUBTRACT STARTUP BEFORE QUOTING A SEARCH RATIO" -- and then gives
you no way to do it. So the subtraction was a manual recipe, which means in
practice it was skipped and whole-process numbers got quoted as search numbers.
That is not a small error here: on this pair the whole-process instruction ratio
reads 0.689 and the startup-subtracted one reads 0.968, because upstream's net
parse costs about 1.7 G instructions to mcfish's 0.7 G.

WHY IT IS WORTH HAVING AT ALL, given `perf_delta.py` exists. That tool subtracts
startup from HARDWARE counters, and its own docstring restricts it to instructions
and macro-ops, because a hardware cycle or miss count carries run-to-run noise
comparable to the term being removed. Callgrind SIMULATES, so every axis here is
deterministic per binary and the subtraction is exact arithmetic on exact numbers.
This is the only startup-clean read of cache and branch behaviour in the tree, and
it is the one instrument that can attribute an IPC gap on a loaded box.

WHAT IT IS NOT. The cache model is callgrind's, not this host's: a fixed
two-level configuration, no prefetcher, no SMT. Ratios between two binaries under
one model are meaningful; the absolute miss counts are not this machine's, and a
sim has already overstated an I-cache effect in this repo by roughly 2x against
direct measurement. Read the ratio, never the absolute.
...
Usage:
  perf_callgrind.sh <A> 16 1 8   # deep    (OUT=a_deep.out)
  perf_callgrind.sh <A> 16 1 1   # startup (OUT=a_shal.out)
  perf_callgrind.sh <B> 16 1 8   # deep    (OUT=b_deep.out)
  perf_callgrind.sh <B> 16 1 1   # startup (OUT=b_shal.out)
  perf_callgrind_delta.py a_deep.out a_shal.out b_deep.out b_shal.out
"""
